(() => {
    ...
    (() => {
        ...
        function rangeInit() {
            ...
            if (rangeItems.length) rangeItems.forEach((rangeItem => {
                ...
                item.noUiSlider.on("update", (function(values, handle) {
                    0 == handle ? fromValue.value = Math.round(values[handle]) : toValue.value = Math.round(values[handle]);
                }));
                let arr = [ null, null ];
                let setValue = (i, value) => {
                    console.log(i);
                    console.log(value);
                    arr[i] = value;
                    console.log(arr);
                ...
                rangeItems.forEach((el => {
                    ...
                    el.addEventListener("change", (e => {
                        if (e.target == fromValue) setValue(0, fromValue.value); else setValue(1, toValue.value);
                    }));
                }));
            }));
        }
        ...
        if (document.querySelector(".goods-page__slider")) new core(".goods-page__slider", {
        ...
        if (document.querySelector(".new-page__slider")) new core(".new-page__slider", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
        ...
        if (document.querySelector(".main-product__preview")) {
            const thumbsSwiper = new core(".thumb-product__slider", {
            ...
            new core(".image-product__slider", {
                modules: [ Navigation, Pagination, Thumb, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                spaceBetween: 10,
        ...
        const adaptSubMenu = document.querySelectorAll(".sub-menu-header");
        if (adaptSubMenu.length) adaptSubMenu.forEach((subMenu => {
            const adaptSubMenuItems = subMenu.querySelectorAll(".sub-menu-header__title").length;
            subMenu.classList.add(`sub-menu-header_${adaptSubMenuItems}`);
        }));
        function documentActions(e) {
            const target = e.target;
            if (target.closest("[data-parent]")) {
                const subMenuTrigger = target.dataset.parent ? target.dataset.parent : null;
                console.log(subMenuTrigger);
                const subMenuModal = document.querySelector(`[data-submenu="${subMenuTrigger}"]`);
                e.preventDefault();
                if (subMenuModal) {
                    ...
                    subMenuModal.classList.toggle("_sub-menu-open");
                }
            }
        }
        window["FLS"] = true;
